 ...
 *
 * [Repeat] (rule 820) is wired generically — see Card.repeatCost (cards/db.ts's parseRepeatCost)
 * and resolvePlayedCard's repeatCount loop (game/moves.ts) — paying it re-invokes this onPlay a
 * second time. Simplification: no player choice of location (see docs/data-sourcing.md) — picks
 * the battlefield with the most enemy units, hitting up to 3 of them; a repeat re-evaluates the
 * same "most enemy units" battlefield rather than letting the player pick a second location.
 */
